...
class RoutingProtocolImpl : public RoutingProtocol {
  public:
    ...
    ~RoutingProtocolImpl();
    typedef std::map<uint16_t, DVCell> DVMap;
    typedef std::pair<uint16_t, uint16_t> DVPair;
    ...
    void generatePingPongMsg();
    void handlePingPong(uint16_t port, void *packet, uint16_t size);
    void recvDATA(uint16_t port, void *packet, uint16_t size);
    void recvDV(uint16_t port, void *packet, uint16_t size);
    void refreshDV();
    void sendDVUpdate(std::vector<DVPair> const &);
    void updateDVTable();
 ...
 private:
    ...
    uint16_t num_ports;
    uint16_t router_id;
    std::map<uint16_t, Port> ports;
    std::map<uint16_t, NNCell> direct_neighbors;
    std::map<uint16_t, uint16_t> forwarding_table;
    std::map<uint16_t, DVCell> dv_map;
};
